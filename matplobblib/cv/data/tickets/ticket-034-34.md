...
2.2. Каждому пикселю карты признаков сопоставляется $$k$$ базовых рамок различных пропорций. Число $$k$$ различается для каждой карты признаков.

3. Составление предсказаний:

3.1. К каждой карте признаков применяется Head, состоящий из двух параллельных сверточных сетей с размером ядра $$3 \\times 3$$.

3.2. Первая сеть предсказывает регрессию: смещение координат и изменение размеров относительно каждой базовой рамки. Выходной тензор имеет размерность $$H\_i \\times W\_i \\times (k \\times 4)$$.

3.2.1. $$H\_i$$ — высота карты признаков.

3.2.2. $$W\_i$$ — ширина карты признаков.

3.2.3. 4 — параметры смещения по осям и изменения высоты с шириной.

3.3. Вторая сеть предсказывает классификацию. Выходной тензор имеет размерность $$H\_i \\times W\_i \\times (k \\times (c + 1))$$.

3.3.1. $$c$$ — количество классов объектов.
...
2.2. C2f ⇔ CSP Bottleneck with 2 convolutions, fast. Входной тензор проходит свертку и разделяется на ветви $$X\_1$$ и $$X\_2$$. Ветвь $$X\_2$$ обрабатывается последовательностью из $$n$$ блоков Bottleneck. Все промежуточные выходы каждого блока Bottleneck сохраняются и конкатенируются вместе с исходными ветвями: $$Concat(X\_1, X\_2, X\_{bot\\\_1}, ..., X\_{bot\\\_n})$$. Алгоритм принудительно направляет градиенты ко всем внутренним слоям, что повышает точность обнаружения мелких объектов.

2.3. C3k2. Модификация блока C2f с возможностью изменения размеров ядер свертки внутри структуры. Размер ядра второго сверточного фильтра внутри Bottleneck является динамическим параметром и может быть увеличен до $$5 \\times 5$$. Локальное увеличение области видимости позволяет пикселям захватывать крупные паттерны без усложнения макроархитектуры.

2.4. SPPF ⇔ Spatial Pyramid Pooling Fast. Блок агрегации контекста. Входной тензор $$X\_0$$ последовательно проходит через три операции MaxPool с окном $$5 \\times 5$$ и шагом 1. Итоговый тензор формируется конкатенацией результатов: $$Concat(X\_0, X\_1, X\_2, X\_3)$$. Математически эквивалентно параллельному применению пулингов с окнами $$5 \\times 5$$, $$9 \\times 9$$ и $$13 \\times 13$$, но требует меньшего числа вычислительных тактов.
