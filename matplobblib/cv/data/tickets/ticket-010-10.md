...
На сглаженном изображении вычисляются магнитуда M(x,y) и направление градиента Θ(x, y). Чаще всего на этом этапе применяется стандартный оператор Собеля. Направление градиента округляется до одного из четырех углов: 0॰, 45॰, 90॰ или 135॰.
...
- Для каждого пикселя алгоритм проверяет, является ли его величина градиента M(x,y) локальным максимумом в направлении градиента Θ(x, y).
