...
изображениями. Перед показом функция сначала восстанавливает известные артефакты
Google Docs (`fractextArea`, `textIoU`, `pcap`, `pcup`, `frac1N`, `int_0^1`,
`sum_i = 1^N`), а также убирает двойное экранирование команд и индексов.
Невосстановимые блоки с `\subscript`, `\superscript`, `\Equation*`,
`\sbracelr` и склеенными командами вроде `\DeltaI` превращаются в обычный
текст без KaTeX delimiters. Корректный простой LaTeX не меняется.
...
Для отдельной строки доступны публичные функции
`repair_markdown_math(markdown)` и `sanitize_markdown_math(markdown)`.
Первая консервативно восстанавливает IoU, AP, mAP и известные токены экспортёра,
вторая переводит оставшиеся опасные math fragments в обычный текст. Обе функции
сохраняют обычный текст, fenced/inline code и Markdown images без изменений.
...
2. Задайте `CFG.EXPORT_PARENT_FOLDER_ID` — ID папки Google Drive, куда можно
   ...
   `CFG.EQUATION_MODE = 'safe-latex'`, чтобы известные структуры
   экспортировались как проверенный LaTeX, а неизвестные — как `[formula]`.
...
`resetCvExport()` и запустите новый экспорт: версия `1.4.x` использует
...
- `safe-latex` — режим по умолчанию: дроби, индексы, скобки, текстовые имена и
  проверенные математические символы преобразуются в валидный `$...$`; любая
  неизвестная структура становится `[formula]`;
- `plain` — распознанные структуры преобразуются в обычный текст без math
  delimiters, неподдерживаемые — в `[formula]`;
